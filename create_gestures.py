...
#creating gesture folger and database file with two columns(g_id, g_name)
def init_create_folder_database():
	# create the folder and database if not exist
	# we create a database to map the classes to their labels
	#right now I'm using a different approach which is the folder structure 
	#to get the class labels with keras
	if not os.path.exists("gestures"):
		os.mkdir("gestures")
	if not os.path.exists("gesture_db.db"):
		conn = sqlite3.connect("gesture_db.db")
		create_table_cmd = "CREATE TABLE gesture ( g_id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, g_name TEXT NOT )"
		conn.execute(create_table_cmd)
		conn.commit()
		
def get_hand_hist():
#open hist file in read mode and store it in f
...
def create_folder(folder_name):
	if not os.path.exists(folder_name): #if it does not exist in the current directory
		os.mkdir(folder_name)

# create the subfolder with name that we give through CLI 		
...
#insert the gesture number(g_id) and gesture name(g_name) in the database
...
def store_images(g_id):
	total_pics = 1200
	#store 1200 empty image if g_id is equal 0
	if g_id == str(0):
	...
	#this include transforming to grayscale+caclBackProject+cvtColor+threshold
	while True:
		img = cam.read()[1]
		img = cv2.flip(img, 1)
		...
		if pic_no == total_pics:
			break
#initialize the folder and database
